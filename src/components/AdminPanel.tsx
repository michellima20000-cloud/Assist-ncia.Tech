...
import {
  ...
  CheckCircle2, AlertCircle, Loader2, ShieldCheck, UserCheck, UserX
} from "lucide-react";
import {
...
export default function AdminPanel({ onBack, onPrintReceipt, onDataChange, currentTheme, onThemeChange, currentUser }: AdminPanelProps) {
...
GARANTIA: ${at.garantia || "Garantia de 90 dias (3 meses)"}`;
  ...
  const handleSelectEmployeeChange = (id: string) => {
    ...
    if (found) {
      setUserForm({
        ...
        name: found.name,
        email: found.email,
        role: found.role === "admin" ? "admin" : "employee",
    ...
    try {
      ...
      if (res.ok) {
        setUserActionMessage({
          type: "success",
          text: userForm.id
            ? `Funcionário "${payload.name}" atualizado com sucesso!`
            : `Novo funcionário "${payload.name}" cadastrado com sucesso!`
        });
        await fetchUsers();
        setSelectedEmp("");
        setUserForm({ id: "", name: "", email: "", role: "employee", password: "" });
        ...
        setUserActionMessage({
          type: "error",
          text: data.error || "Erro ao salvar funcionário. Verifique os dados informados."
        });
      }
    } catch (err) {
      console.error(err);
      setUserActionMessage({
        type: "error",
        text: "Erro de comunicação ao salvar funcionário."
      });
    } finally {
  ...
  const requestDeleteEmployee = (emp: { id: string; name: string; email: string; role?: string }) => {
    if (!emp.id) return;
    setUserActionMessage(null);
    if (emp.id === "u-1" || emp.email.toLowerCase() === "michel.lima20000@gmail.com") {
      setUserActionMessage({
        type: "error",
        text: "A conta do Administrador principal não pode ser excluída por motivos de segurança!"
      });
      return;
    }
    if (currentUser && (emp.id === currentUser.id || emp.email.toLowerCase() === currentUser.email?.toLowerCase())) {
      setUserActionMessage({
        type: "error",
        text: "Você não pode excluir sua própria conta enquanto estiver conectado a ela."
      });
      return;
    }
  ...
  const handleConfirmDeleteEmployee = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setUserActionMessage({
          type: "success",
          text: `A conta de "${employeeToDelete.name}" foi excluída com sucesso!`
        });
        if (selectedEmp === employeeToDelete.id || userForm.id === employeeToDelete.id) {
          setSelectedEmp("");
          setUserForm({ id: "", name: "", email: "", role: "employee", password: "" });
        }
        ...
        setUserActionMessage({
          type: "error",
          text: data.error || "Não foi possível excluir a conta. Tente novamente."
        });
      }
    } catch (error: any) {
      ...
      setUserActionMessage({
        type: "error",
        text: "Erro de comunicação com o servidor ao excluir a conta."
      });
    } finally {
    ...
    <div className="space-y-4 max-w-4xl mx-auto pb-10">
      ...
      {activeMenu === 'users' && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-5">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Gerenciamento de acessos, administradores e atendentes</p>
                </div>
              </div>
              <button
                onClick={() => {
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                Escolha o Funcionário para Editar
              </label>
              <select
                value={selectedEmp}
                onChange={(e) => handleSelectEmployeeChange(e.target.value)}
                className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-blue-500 focus:bg-white transition"
              >
                <option value="">-- CADASTRAR NOVO FUNCIONÁRIO --</option>
                {employees.map(e => (
                  <option key={e.id} value={e.id}>
                    {e.name} ({e.role === 'admin' ? 'Administrador' : 'Atendente'})
                  </option>
                ))}
              </select>
            </div>

            <form onSubmit={handleSaveEmployee} className="p-4 border border-blue-100 bg-blue-50/20 rounded-2xl space-y-4 text-xs">
              <div className="flex items-center justify-between border-b border-blue-100/60 pb-2">
                <span className="font-bold text-slate-700">
                  {userForm.id ? `Editando: ${userForm.name}` : "Dados do Novo Funcionário"}
                </span>
                {userForm.id && (
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none focus:border-blue-500 transition"
                  />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-slate-600 font-medium mb-1">Nível de Acesso</label>
                  <select
                    value={userForm.role}
                    onChange={(e) => setUserForm({ ...userForm, role: e.target.value as "admin" | "employee" })}
                    className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none focus:border-blue-500 transition"
                  >
                    <option value="employee">Funcionário comum (Atendente)</option>
                    <option value="admin">Administrador</option>
                  </select>
                </div>
                <div>
                  <label className="block text-slate-600 font-medium mb-1">
                    Senha {userForm.id && <span className="text-slate-400 font-normal">(deixe em branco para não alterar)</span>}
                  </label>
                  <input
                    type="password"
                    value={userForm.password}
                    onChange={(e) => setUserForm({ ...userForm, password: e.target.value })}
                    placeholder={userForm.id ? "Senha inalterada" : "Digite a senha de acesso"}
                    className="w-full p-2.5 bg-white border border-slate-200 rounded-lg outline-none focus:border-blue-500 transition"
                  />
                </div>
              </div>

              <div className="flex gap-2 justify-end pt-2 border-t border-blue-100/60">
                {userForm.id && userForm.id !== "u-1" && userForm.email.toLowerCase() !== "michel.lima20000@gmail.com" && (
                  <button
                    type="button"
                    onClick={() => requestDeleteEmployee(userForm)}
                    className="px-4 py-2 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white font-bold rounded-lg transition flex items-center gap-1.5 shadow-sm"
                  >
                    <Trash2 className="w-4 h-4" />
                    Excluir Funcionário
                  </button>
                ...
                <button
                  ...
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 disabled:opacity-50 text-white font-bold rounded-lg transition flex items-center gap-1.5 shadow-sm"
                >
                  {isSavingEmployee ? (
                    <>
          ...
          {/* LISTA COMPLETA DE FUNCIONÁRIOS E CONTAS */}
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h4 className="font-bold text-sm text-slate-800">
                  Funcionários e Usuários do Sistema ({employees.length})
                </h4>
                <p className="text-xs text-slate-400">
                  Lista de todas as contas registradas para acesso ao sistema
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="divide-y divide-slate-100">
              {employees.length === 0 ? (
                <div className="py-6 text-center text-xs text-slate-400">
                  Nenhum funcionário cadastrado além do administrador principal.
                </div>
              ) : (
                employees.map(emp => {
                  const isPrimaryAdmin = emp.id === "u-1" || emp.email?.toLowerCase() === "michel.lima20000@gmail.com";
                  const isCurrent = currentUser && (emp.id === currentUser.id || emp.email?.toLowerCase() === currentUser.email?.toLowerCase());

                  return (
                    <div
                      key={emp.id}
                      className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50/60 p-2 rounded-xl transition"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold shrink-0 ${
                          ...
                          {emp.name ? emp.name.slice(0, 2).toUpperCase() : "US"}
                        </div>
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isPrimaryAdmin && (
                              <span className="text-[10px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-bold flex items-center gap-0.5">
                                <ShieldCheck className="w-3 h-3" /> Admin Principal
                              </span>
                            )}
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                              ...
                              {emp.role === "admin" ? "Administrador" : "Atendente"}
                            </span>
                          </div>
                          <p className="text-[11px] text-slate-500 truncate mt-0.5">
                            {emp.email}
                            {(emp as any).phone ? ` • ${(emp as any).phone}` : ""}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                        ...
                        {!isPrimaryAdmin && !isCurrent && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 hover:text-red-700 font-bold rounded-lg text-xs transition flex items-center gap-1 border border-red-200"
                            title={`Excluir definitivamente a conta de ${emp.name}`}
                          >
          ...
          {employeeToDelete && (
            <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-150">
              <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-150 space-y-4">
                ...
                <div className="p-3.5 bg-red-50/70 border border-red-100 rounded-xl space-y-2 text-xs text-slate-700">
                  <p>
                    Tem certeza de que deseja excluir permanentemente a conta de:
                  </p>
                  <div className="font-bold text-slate-900 bg-white p-2.5 rounded-lg border border-red-200">
                    <p>{employeeToDelete.name}</p>
                    <p className="text-[11px] font-normal text-slate-500">{employeeToDelete.email}</p>
                    <p className="text-[10px] text-red-600 font-semibold mt-1">
                      Nível: {employeeToDelete.role === "admin" ? "Administrador" : "Atendente"}
                    </p>
                  </div>
                  <p className="text-slate-500 text-[11px]">
                    ⚠️ Esta ação removerá imediatamente a conta do banco de dados e o funcionário não conseguirá mais efetuar login no sistema.
                  </p>
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-red-600 hover:bg-red-700 active:bg-red-800 disabled:opacity-50 text-white font-bold rounded-xl text-xs transition flex items-center gap-1.5 shadow-sm"
                  >
                    {isDeletingEmployee ? (
                      <>
                        <Loader2 className="w-4 h-4 animate-spin" />
